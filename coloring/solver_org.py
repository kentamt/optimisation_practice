...
class Vertex:
    def __init__(self, name : int, n_colors : int):
        ...
        self.n_neighbors = 0
        # self.n_available_colors = sum(self.available_colors) # 塗れる色の個数
    
    def get_available_colors(self) -> int :
        return sum(self.available_colors) # 残り塗れる色の個数
...
def solve_it(input_data):
    ...
    for _ in range(1000):
        ...
        while sum([e.is_visited for e in sorted_nodes]) != node_count:
            ...
            # バリュアブルオーダー
            valuable_nodes = sorted([e for e in G.vertex_dict.values() if not e.is_visited], key=lambda x:x.get_available_colors())
            # print("valuable order:")
            # for e in valuable_nodes:
            #     print(e.name, e.get_available_colors())
            # print("---")

            cv = valuable_nodes[0] # 一番塗る色が少ないもの
