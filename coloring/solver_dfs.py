...
def solve_it(input_data):
    ...
    while loop_count != 0:
        ...
        random_node = sorted([e for e in G.vertex_dict.values()], key=lambda x:x.n_neighbors) #, reverse=True)
        ...
        def dfs(cv, visited):
            ...
            # 毎回、solutionにしたがって全ノードのavailabityをチェック
            # TODO: 更新分だけやるのがはやいが面倒なのでこれでいく
            for node, color in solution:
                # print("fill availablity:", node.name, color)            
        ...
        # for nid in random_node_name:
            # v = G.vertex_dict[nid]        
        for v in random_node:
            visited = []
            if not v.is_visited:
                dfs(v, visited)
            # if visited:
            #     print(f"start from {v}")
            #     s = 0
            #     for e in visited:
            #         print(f"{e}", end=" ")
            #         s += e.name
        ...
        if value < min_colors:
            ...
            # print(min_colors, solution)
    
    opt = 1
    
    # prepare the solution in the specified output format
    output_data = str(min_colors) + ' ' + str(opt) + '\n'
